...
const checkMakeDir = (path: String, callback: Function) => {
    fs.stat(path, <T>(error: T, stats: any) => {
        if (error)
...
rl.on("line", function <T>(line: T) {
    ...
    if (cmd === 'exit()') {
        ...
        if (cmd === 'tgit init') {
            ...
            fs.writeFile(`${__dirname}/tgit.log`, '', 'utf8', function () { });
        ...
        if (isInit) {
            if (cmd.includes('mkdir ')) { // 폴더 생성
                if (cmd.includes('mkdir TGIT')) {
                    ...
                    checkMakeDir(path, <T>(error: T, isTrue: T) => {
                        if (error) {
                ...
                fs.readdir('./', function <T>(filelist: T) {
                ...
                if (cmd.includes('touch TGIT')) {
                    ...
                    fs.writeFile(path, '', function () {
                        if (fs.readdir('./', function (filelist: any) { filelist.includes(path.replace('touch ', '')) })) {
                ...
                fs.readFile(path, 'utf8', <T>(error: T, data: T) => {
                    if (error) {
                ...
                if (cmd.includes('tgit write TGIT')) {
                    ...
                    console.log(`\u001b[41mTypeGit Error 1010 :: tgit.log의 내용을 변경할 수 없습니다.\u001b[0m\n`);
                    process.stdout.write(`${__dirname} (TGIT)\n> `);
                } else {
                    ...
                    rl.question("", <T>(line: T) => {
                        content = String(line);
                        console.log(`\u001b[32mTypeGit : 성공적으로 입력되었습니다.\u001b[0m`);
                        fs.writeFile(path, content, 'utf8', function <T>(error: T) {
                            if (error) {
                ...
                if (cmd.includes('tgit rename TGIT')) {
                    ...
                    let folder: string, newFolder: string;
                    rl.question("폴더명 : ", <T>(line: T) => {
                        folder = `${__dirname}/${String(line)}`;

                        rl.question("변경할 폴더명 : ", <T>(line: T) => {
                            newFolder = `${__dirname}/${String(line)}`;

                            fs.rename(folder, newFolder, function <T>(error: T) {
                                if (error) {
                ...
                rl.question("\u001b[32mTypeGit : 업로드하는 author의 이름을 알려주세요.\u001b[0m\n", <T>(line: T) => {
                    ...
                    rl.question("\u001b[32mTypeGit : 업로드하는 author의 email을 입력해주세요.\u001b[0m\n", <T>(line: T) => {
                        ...
                        fs.writeFile(`${__dirname}/TGIT/${cmd.replace('tgit upload ', '')}`, '', function <T>(error: T) {
                            if (error) {
                                console.log(`\u001b[41mTypeGit System Error :: ${error}.\u001b[0m\n`);
                                process.stdout.write(`${__dirname} (TGIT)\n> `);
                            }
                        });
                        fs.copyFile(path, `${__dirname}/TGIT/${cmd.replace('tgit upload ', '')}`, <T>(error: T) => {
                            if (error) {
                                console.log(`\u001b[41mTypeGit Error 1008 :: 해당하는 파일을 찾을 수 없습니다.\u001b[0m\n`);
                                process.stdout.write(`${__dirname} (TGIT)\n> `);

                                fs.unlink(`${__dirname}/TGIT/${cmd.replace('tgit upload ', '')}`, () => {
                                ...
                                fs.appendFile(`${__dirname}/tgit.log`, `\n${JSON.stringify(TGIT_STACK)}`, function <T>(error: T) {
                                    if (error) {
                                        console.log(`\u001b[41mTypeGit System Error :: ${error}.\u001b[0m\n`);
                                        process.stdout.write(`${__dirname} (TGIT)\n> `);
                                    }
                ...
                fs.unlink(TGIT_STACK.UPLOAD, <T>(error: T) => {
                    if (error) {
                        console.log(`\u001b[41mTypeGit Error 1015 :: 해당 명령어를 읽을 수 없습니다.\u001b[0m\n`);
                        process.stdout.write(`${__dirname} (TGIT)\n> `);
                    } else {
